 ...
 */
function faac_get_page_by_title( $page_title, $output = OBJECT, $post_type = 'page' ) {
...
function custom_rewrite_rule()
{
  ...
  $pageProduct = faac_get_page_by_title($titlePageProduct);
  $pageCategory = faac_get_page_by_title($titlePagecategory);
...
add_filter(
  ...
  function ($caps, $cap) {
      ...
      if ($cap === 'unfiltered_html' && !empty($user->roles) && in_array('administrator', $user->roles, true) ) {
...
function umc_custom_kses_init() {
  $user = wp_get_current_user();
  if ( current_user_can( 'unfiltered_html' && in_array('administrator', $user->roles, true)) ) {
    kses_remove_filters();
  }
}
...
require get_template_directory() . '/inc/api-cache.php';
...
// Aggiunge la colonna "Data di registrazione"
add_filter('manage_users_columns', function($columns) {
    $columns['registration_date'] = 'Data di registrazione';
    return $columns;
});

// Popola la colonna "Data di registrazione"
