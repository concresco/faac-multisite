...
$cache_buster = time(); // Add a cache buster variable

$sidebar_list = json_decode(file_get_contents(API_PIM . 'sidebar?lang=' . $currentLanguage . '&cb=' . $cache_buster), true);
...
$product = json_decode(file_get_contents(API_PIM . 'product?' . 'lang=' . $currentLanguage . '&product_id=' . $productID . '&cb=' . $cache_buster), true);
    ...
    foreach($product['relatedProductsSection']['accordion'] as $key => $item){
        if($item['accordionBody'] == 1){
            ...
            foreach($product['relatedProductsSection']['accordion'][$key]['productsList'] as $itemp){
                ...
                $productRelated = json_decode(file_get_contents(API_PIM . 'product?' . 'lang=' . $currentLanguage . '&product_id=' . $itemp['product_id'] . '&cb=' . $cache_buster), true);
